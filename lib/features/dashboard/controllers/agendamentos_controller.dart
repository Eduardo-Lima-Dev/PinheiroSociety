...
class AgendamentosController extends ChangeNotifier {
  ...
  List<String> get horariosPaginados {
    ...
    return horariosDisponiveis.sublist(inicio,
  ...
  Future<void> carregarDadosAgendamentos() async {
    ...
    try {
      ...
      if (reservasResp['success'] == true) {
        ...
        reservas =
            todasReservas.where((r) => r['status'] != 'CANCELADA').toList();
        print(
            'DEBUG: Carregadas ${reservas.length} reservas para $dataFormatada');
        if (reservas.isNotEmpty) {
          print('DEBUG: Primeira reserva: ${reservas[0]}');
          print(
              'DEBUG: Campo duracaoMinutos da primeira reserva: ${reservas[0]['duracaoMinutos']}');
        }
      }

      // PROJEÇÃO DE RECORRÊNCIAS: caso o backend não retorne instâncias futuras
      await _projetarRecorrenciasParaDia(dataSelecionada);
      ...
      if (quadras.isNotEmpty) {
        print(
            'DEBUG: Primeira quadra horários: ${quadras[0]['horarios']?.length ?? 0}');
      }
    } catch (e) {
  ...
  Future<void> _projetarRecorrenciasParaDia(DateTime data) async {
    try {
      final respTodas = await QuadraRepository.getReservas();
      if (respTodas['success'] != true) return;

      final todas = List<Map<String, dynamic>>.from(respTodas['data'] ?? []);
      final diaSemanaSelecionado = data.weekday; // 1=Mon ... 7=Sun
      final dataISO = _formatarDataParaAPI(data);

      // Para cada reserva recorrente ativa, se casar com o dia da semana e dentro do período, projeta
      for (final r in todas) {
        final observacoes = (r['observacoes'] as String?)?.toLowerCase() ?? '';
        final recorrente = r['recorrente'] == true ||
            observacoes.contains('mensalista') ||
            observacoes.contains('fixo');
        if (!recorrente) continue;
        if (r['status'] == 'CANCELADA') continue;

        final dataInicioStr = (r['data'] as String?) ?? '';
        if (dataInicioStr.isEmpty) continue;

        // Comparações por string ISO yyyy-MM-dd (backend envia assim)
        final diaSemanaReserva =
            (r['diaSemana'] as int?) ?? DateTime.parse(dataInicioStr).weekday;
        if (diaSemanaReserva != diaSemanaSelecionado) continue;

        final dataFim =
            r['dataFimRecorrencia'] as String?; // pode ser null (sem fim)

        final bool dentroDoPeriodo = (dataISO.compareTo(dataInicioStr) >= 0) &&
            (dataFim == null || dataISO.compareTo(dataFim) <= 0);
        if (!dentroDoPeriodo) continue;

        // Se já existir reserva concreta para o horário/quadra neste dia, não projetar
        final jaExiste = reservas.any(
            (e) => e['quadraId'] == r['quadraId'] && e['hora'] == r['hora']);
        if (jaExiste) continue;

        // Criar ocorrência sintética para exibição no grid. Usa o id do pai
        // para abrir detalhes quando clicado.
        final sintetica = {
          'id': r['reservaPaiId'] ?? r['id'],
          'cliente': r['cliente'],
          'quadraId': r['quadraId'],
          'hora': r['hora'],
          'data': dataISO, // data da ocorrência projetada
          'status': 'ATIVA',
          'observacoes':
  ...
  void _montarDadosGrid() {
    print(
        'DEBUG: Montando grid com ${quadras.length} quadras e ${reservas.length} reservas');
    ...
    for (var quadra in quadras) {
      ...
      for (final horario in horariosDisponiveis) {
        ...
        final reserva = reservas.firstWhere(
          (r) {
            ...
            final horasDuracao =
                (duracaoMinutos / 60).ceil(); // Quantas horas ocupa

            // Log para debug de todas as reservas desta quadra
            if (horaInt == horaInicio) {
              print(
                  'DEBUG: Reserva encontrada - Quadra: $quadraId, Hora início: $horaInicio, duracaoMinutos do JSON: ${r['duracaoMinutos']}, duracaoMinutos calculado: $duracaoMinutos, horasDuracao: $horasDuracao');
            }

            // Verifica se o horário atual está dentro do range da reserva
            final ocupaEsteHorario =
                horaInt >= horaInicio && horaInt < (horaInicio + horasDuracao);
      ...
      print(
          'DEBUG: Quadra ${quadra['nome']}: ${horariosQuadra.length} horários montados');
    }
  }
  ...
  String _determinarStatusReserva(Map<String, dynamic> reserva) {
    ...
    final observacoes =
        (reserva['observacoes'] as String?)?.toLowerCase() ?? '';
